...
SendMode, Input  ; Use SendInput mode for faster key sending
SetBatchLines, -1
SetWorkingDir, %A_ScriptDir%

; Tray icon Base64 images
...
; Set the initial tray icon to "off"
Menu Tray, Icon, % "HBITMAP:*" hICON_off := CreateBitMap(Base64PNG_off)
...
MButton::
    ...
    if (toggle) {
        ...
        Menu Tray, Icon, % "HBITMAP:*" hICON_on := CreateBitMap(Base64PNG_on)
    } else {
...
; Define the coordinates and color for the "a" key press logic
slot1_isabuff := true
slot1_coords := "507, 1338"
slot1_coords := StrSplit(slot1_coords, ", ") ; Split the string into x and y coordinates
PixelGetColor, slot1activecolor, % slot1_coords[1], % slot1_coords[2], RGB ; Use the coordinates to get the color
...
~LButton::
    if (toggle && WinActive("ahk_exe Diablo IV.exe")) {
        ...
        ; Send two Left Mouse button clicks immediately
        Click
        Sleep, 30  ; Small delay to ensure the first click registers
        Click
        Sleep, 30  ; Small delay to ensure the second click registers
        ...
        Loop {
            ...
            ; Check slot 1 color and send "a" key press if conditions are met
            PixelGetColor, color, % slot1_coords[1], % slot1_coords[2], RGB
            ;ToolTip, Color: %color%`nSlot Active Color: %slot1activecolor%  ; Show the colors being checked for debugging
            if ((color = slot1activecolor) && slot1_isabuff) {
                SendInput, a
                Sleep, 10  ; Small delay to ensure the "a" key press registers
            }
        }
    }
return

; Function to create bitmap from Base64 string (used for tray icon)
CreateBitMap(ByRef strB64) {
    VarSetCapacity(B64, StrLen(strB64) << !!A_IsUnicode) ; Not needed on short string
